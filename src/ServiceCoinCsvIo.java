...
import jp.nyatla.nccdb.table.CoinUrlTable;
 ...
 * ServiceCoinPairCSVは{@link CoinMasterTable},{@link CoinUrlTable},{@link IdPairTable}と関連付けられます。
...
public class ServiceCoinCsvIo
{
	private static void importCSV(ArgHelper ap,SqliteDB i_db) throws SdbException
	{
		...
		try {
			...
			//列インデックスを得る。	
			int coin_symbol_idx	=csv.getIndex(CoinUrlTable.DN_name);
			int coin_name_idx	=csv.getIndex(CoinUrlTable.DN_id_coin_url_type);
			//サービス名インデクス、その個数を得る
